...
if (!isset($_SESSION['user_id']) || $_SESSION['user_role'] !== 'doctor') { header("Location: ../login.php"); exit(); }

// معالجة الحذف
if (isset($_GET['delete'])) { $pdo->prepare("DELETE FROM appointments WHERE id = ?")->execute([$_GET['delete']]); header("Location: manage_appointments.php"); exit(); }

// معالجة تحديث الحالة والملاحظات
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_appointment'])) {
        $pdo->prepare("INSERT INTO appointments (patient_id, appointment_date, status, medical_notes) VALUES (?, ?, 'pending', ?)")
             ->execute([$_POST['patient_id'], $_POST['date'], $_POST['notes']]);
    }
...
<body>
    ...
    <div class="content">
        <div class="card p-4">
            ...
            <table class="table table-hover align-middle">
                <thead class="table-dark"><tr><th>المريض</th><th>التاريخ</th><th>الحالة</th><th>الملاحظات الطبية</th><th>إجراء</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST"><input type="hidden" name="appointment_id" value="<?php echo $a['id']; ?>">
                                <textarea name="notes" class="form-control" onblur="this.form.submit()"><?php echo $a['medical_notes']; ?></textarea>
                                <input type="hidden" name="update_notes" value="1"></form>
                        </td>
                        <td><a href="?delete=<?php echo $a['id']; ?>" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></a></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <div class="modal fade" id="addApptModal" tabindex="-1"><div class="modal-dialog"><form method="POST" class="modal-content"><div class="modal-header"><h5 class="modal-title">إضافة موعد</h5></div><div class="modal-body"><select name="patient_id" class="form-control mb-2"><?php foreach ($patients as $p): ?><option value="<?php echo $p['id']; ?>"><?php echo $p['name']; ?></option><?php endforeach; ?></select><input type="datetime-local" name="date" class="form-control mb-2" required><textarea name="notes" class="form-control" placeholder="ملاحظات.."></textarea></div><div class="modal-footer"><button type="submit" name="add_appointment" class="btn btn-primary">حفظ</button></div></form></div></div>
</body>
</html>